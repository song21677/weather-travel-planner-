...
const $search = document.getElementById("places");

const routes = {
	// "/plan" : Place,
	"/:area/:category" : PlaceList,
	"/:keyword" : PlaceList
}

const changeUrl = (requestedUrl, ...data) => {
    history.pushState(null, null, requestedUrl);
    
    const planForm = requestedUrl.match(/^\/plan$/);
    const areaAndCategory = requestedUrl.match(/^\/([^/]+)\/([^/]+)$/);
    const keyword = requestedUrl.match(/^\/([^/]+)$/);
    
    if (planForm) {
	// $search.innerHTML = routes["/plan"].template();
    } else if (areaAndCategory) {
	let area = areaAndCategory[1];
	let category = areaAndCategory[2];
	$search.innerHTML = routes["/:area/:category"].template(data);
	addEventSelectButton();
    } else if (keyword) {
	let keyword = keyword[1];
	$search.innerHTML = routes["/:keyword"].template(data);
	addEventSelectButton();
    }
}
...
//        })
        ...
        const places = await fetchURI(`/places?area=${area}&category=${category}`);
        changeUrl(`/${area}/${category}`, places);
    });
});
...
function addEventApplyButton() {
    ...
    button.addEventListener('click', function() {
        ...
        setBlock(givemedate, date, convertTo24HourFormat(startTime), convertTo24HourFormat(endTime), placeNo, placeName, placeCategory);
    });
}
...
function setBlock(givemedate, date, startHour, endHour, placeNo, placeName, placeCategory) {
    let color;
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            updatePlanDTO(date, startHour, endHour, placeNo, placeName, placeCategory, color);     
        },
...
var DetailPlan = (function() {
    function DetailPlan(date, startHour, endHour, placeId, placeName, placeCategory, placeColor) {
        this.date = date;
...
function updatePlanDTO(date, startHour, endHour, placeNo, placeName, placeCategory, color) {
    ...
    const detailPlan = new DetailPlan(date, startHour, endHour, placeNo, placeName, placeCategory, color);
    plan.addDetailPlan(detailPlan);
    
    sessionStorage.setItem('plan', JSON.stringify(plan.get()));
    
    const planHTML = fetchHTML('/planForm');
    document.querySelector('.frame').innerHTML = planHTML;
    removeCategories();
    resizeMap();
    deleteMapScript();
    loadMapScript();
